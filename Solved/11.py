...
Problem 11:
    ...
    marked in red. (- on either side of number since all one color)
...
for i in range(20):
    for j in range(17):
        maxProduct = max(M[i][j]*M[i][j+1]*M[i][j+2]*M[i][j+3],maxProduct)
        maxProduct = max(M[j][i]*M[j+1][i]*M[j+2][i]*M[j+3][i],maxProduct)

for i in range(17):
    for j in range(17):
        maxProduct = max(M[i][j]*M[i+1][j+1]*M[i+2][j+2]*M[i+3][j+3],maxProduct)
        maxProduct = max(M[i][19-j]*M[i+1][18-j]*M[i+2][17-j]*M[i+3][16-j],maxProduct)
